import { LocalIDClient } from '../../src/LocalIDClient';
import { LocalIDError } from '../../src/types';
...
describe('LocalIDClient with backend config', () => {
  ...
  it('without backend config, no fetch calls are made', async () => {
    const clientNoBe = new LocalIDClient(
...
describe('LocalIDClient.initialize()', () => {
  ...
  it('fetches and stores localidPublicKey and signingKey', async () => {
    const client = new LocalIDClient(
      { appId: 'com.test', returnScheme: 'test', backend: backendConfig },
      mockOpenUrl,
    );
    ...
    const priv = client as unknown as { localidPublicKey: string | null; signingKey: string | null };
